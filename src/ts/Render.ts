...
export class Render {
   ...
   */
  private renderWithChildren = (elementName: string):Function => (...classes: string[]): Function => {
   ...
   */
  private renderWithoutChildren = (elementName: string):Function => (...classes: string[]): HTMLElement => {
    let htmlelement = document.createElement(elementName)
    if (classes.length > 0) {
      htmlelement.classList.add(...classes)
    }
